...
//Агрегация — связи типа "часть-целое", где один объект содержит другой,
//но объекты могут существовать независимо друг от друга. Если "целое" уничтожается, "часть" продолжает существовать.

//Ассоциация — это общая связь между объектами, где они взаимодействуют, но не имеют строгой
//зависимости или иерархии "часть-целое". Связь может быть одно- или двунаправленной.

//Наследование — это иерархическая связь, где один класс (дочерний) наследует свойства и методы другого класса (родительского).

//Зависимость — это связь, где один объект временно использует другой для выполнения задачи,
//но не содержит его как поле и не наследует.
...
namespace ChatBot
{
    ...
    public partial class ChatBot : Form //Пример наследования
    {
        //Экземпляры необходимы для того, чтобы код модулей мог получить общий доступ к данным и компонентам.
        // Экземпляр модуля логики бота
        private ModuleBot bot; //Пример агрегации
        ...
        public ChatBot()
        {
            ...
            using (Login loginForm = new Login()) //Пример ассоциации, потому что ChatBot временно использует
            //Login для получения имени пользователя (userName), но не содержит его как постоянную часть.
            //После закрытия формы Login связь между объектами разрывается и они существуют независимо
            {
            ...
            bot = new ModuleBot(userName); //Пример агрегации
            //Класс чат бота содержит экземпляр ModuleBot как свою часть для обработки логики бота.
            //Если объект форма уничтожается или закрывается, то ModuleBot не уничтожается автоматически —
            //он просто перестаёт использоваться 
        ...
        private async void button1_Click(object sender, EventArgs e)
        {
            ...
            string response = await bot.GetResponseAsync(userInput); //Пример зависимости для получения ответа бота
            //использует его функциональность, но не владеет им.
